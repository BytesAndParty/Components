...
- [ ] **Dock** -- macOS-style Magnification-Nav für Mobile Bottom-Bar → `components/dock/` *(geplant)*

## Hero / Landing

- [ ] **Parallax Hero Image** -- Scroll-gesteuerte Tiefe
- [ ] **Animated CTA Button** -- Shimmer-Border + Shiny-Text; siehe `border-beam` + `shiny-text` *(geplant)*
...
- [ ] **CursorGlow** -- Leichter CSS-Cursor-Glow (dauerhaft, Layout-Level) → `components/cursor-glow/` *(geplant)*
- [x] **Particles** -- Canvas-basiert, Mouse-Repulsion → `components/particles/`
- [ ] **ParticlesCard** -- Particles als Card-Hintergrund-Wrapper → `components/particles/particles-card.tsx` *(geplant, ~30 Zeilen)*
- [ ] **BlurFade** -- Viewport-Einblend-Wrapper (blur + opacity) → `components/blur-fade/` *(geplant)*
...
- [ ] **AuroraText `variant="gradient"`** -- Stetiger Gradient-Loop als CTA-Variante *(Erweiterung bestehend)*
...
- [ ] **MorphingText** -- CSS-Blur-Überblend zwischen Strings → `components/morphing-text/` *(geplant)*
- [ ] **ShinyText** -- Metallic Shine-Animation + ShinyButton CTA → `components/shiny-text/` *(geplant)*
...
- [ ] **Backlight `interactive`-Prop** -- Cursor-Tracking zusätzlich zu Blob-Animationen *(Erweiterung bestehend)*
...
- [ ] **NumberTicker** -- Slot-Rollen bei Zahlenwechsel; Integration in CartIcon-Badge → `components/number-ticker/` *(geplant)*
- [ ] **CircularProgress** -- SVG-Kreis-Fortschrittsanzeige für Checkout-Flow → `components/circular-progress/` *(geplant)*

## Buttons & Interactive

- [x] **MagneticButton** -- Magnetischer Hover-Effekt, 7 Varianten → `components/magnetic-button/`
- [ ] **MagneticButton `variant="beam"`** -- Rotierender Border-Beam → *(Erweiterung bestehend)*
...
- [ ] **Logo Ticker / Marquee** -- Infinite scroll, pause-on-hover → bereits via `VelocityScroll` abgedeckt
- [ ] **AnimatedCounter** -- Number-roll "1.234+ Kunden" → `components/number-ticker/` *(geplant, selbe Komponente wie CartIcon-Badge)*
...
- [ ] **FileTree** -- Rekursiver Dateibaum mit Expand/Collapse → `components/file-tree/` *(geplant)*
