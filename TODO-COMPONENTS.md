...
const images = [ "https://picsum.photos/400/400?grayscale",
"https://picsum.photos/500/500?grayscale",
"https://picsum.photos/600/600?grayscale",
"https://picsum.photos/700/700?grayscale",
"https://picsum.photos/300/300?grayscale" ];

const transformStyles = [ "rotate(5deg) translate(-150px)", "rotate(0deg)
translate(-70px)", "rotate(-5deg)", "rotate(5deg) translate(70px)",
...
bun add gsap damit man sich weitere bilder davon anschauen kann klickt man dann
auf das bild, wird es darüber in groß angezeigt, dh das bild in der gallery
darüber ändert sich

den stepper könnten wir verwenden bei unserer wein webseite weil es eine
komplexere logik beim bestellen der personalisierten etiket möchte, ich muss ja
wissen welcehs etiket ih auf welche frlasche geben möchte. man zahlt nur pro
etiket. dh ich muss beiim bestellen vom etikett entweder ein item aus dem
warenkorb auswählen oder ein oder mehrere weinflaschen
https://reactbits.dev/components/stepper import Stepper, { Step } from
'./Stepper';
...
particles könnte man nur tesweise mal auf eine card verwenden
https://reactbits.dev/backgrounds/particles import Particles from './Particles';
...
ich möchte eine reihe haben, mit insgesamt 3 weinen das default bild liegt in:
_public_/wine-default.png bun add ogl on hover möchte ich diesen effekt haben:
...
das gefällt mir auch sehr gut aber ev sollten wir das auch mal nur auf eine
eigenen card oder einer section testen:
https://reactbits.dev/animations/click-spark import ClickSpark from
'./ClickSpark';

<ClickSpark sparkColor='#fff' sparkSize={10} sparkRadius={15} sparkCount={8}
duration={400}

> {/_ Your content here _/} </ClickSpark>

bun x --bun shadcn@latest add @react-bits/ClickSpark-JS-CSS

selbes mit dem bun x --bun shadcn@latest add @react-bits/SplashCursor-JS-CSS
...
function SplashCursor({ SIM_RESOLUTION = 128, DYE_RESOLUTION = 1440,
CAPTURE_RESOLUTION = 512, DENSITY_DISSIPATION = 3.5, VELOCITY_DISSIPATION = 2,
PRESSURE = 0.1, PRESSURE_ITERATIONS = 20, CURL = 3, SPLAT_RADIUS = 0.2,
SPLAT_FORCE = 6000, SHADING = true, COLOR_UPDATE_SPEED = 10, BACK_COLOR = { r:
0.5, g: 0, b: 0 }, TRANSPARENT = true }) { const canvasRef = useRef(null); const
animationFrameId = useRef(null);
