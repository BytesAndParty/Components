/**
 * AccentSwitcher — Variant B: Dual SVG + currentColor Trick
 *
 * Two stacked SVGs swapped via CSS opacity on hover.
 * Colored SVG uses <g style={{ color: oklch }}> + fill="currentColor"
...
const svgProps = {
	...
	fill: 'none', strokeWidth: 2, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const,
};

const palettePath = 'M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z';
...
// Unique class prefix to avoid collisions
const CLS = 'as-dual';

export function AccentSwitcherDualSvg({
	palettes,
	defaultPalette,
	activePalette,
	...
	useEffect(() => {
		...
		const observer = new MutationObserver(() => {
			...
			if (val && palettes[val]) setAccent(val);
		});
		observer.observe(document.documentElement, { attributes: true, attributeFilter: [accentAttribute] });
	...
	return (
		<div className={className} style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', ...style }}>
			<style>{`
				.${CLS}-btn { background: transparent; transition: background 0.15s; }
				.${CLS}-btn:hover { background: rgba(255,255,255,0.08); }
				.${CLS}-mono { transition: opacity 0.3s ease; }
				.${CLS}-color { position: absolute; inset: 0; opacity: 0; transition: opacity 0.3s ease; }
				.${CLS}-btn:hover .${CLS}-mono { opacity: 0; }
				.${CLS}-btn:hover .${CLS}-color { opacity: 1; }
			`}</style>
			...
			<button
				...
				style={{
					...
					border: 'none', color: 'inherit', cursor: 'pointer', position: 'relative',
				}}
			>
				{/* Layer 1: Monochrome (default) */}
				<svg {...svgProps} className={`${CLS}-mono`} stroke="currentColor">
					<path d={palettePath} />
					{dots.map((d, i) => (
				...
				{/* Layer 2: Colored dots (hover) — uses CSS color + currentColor trick */}
				<svg {...svgProps} className={`${CLS}-color`} stroke="currentColor">
					<path d={palettePath} />
					{dots.map((d, i) => (
						<g key={i} style={{ color: dotColors[i] }}>
							<circle cx={d.cx} cy={d.cy} r={1} fill="currentColor" />
						</g>
					))}
				</svg>
			</button>
			...
			{open && (
				<div ref={menuRef} style={{
					...
					minWidth: '11rem', borderRadius: '0.5rem', border: '1px solid var(--border)',
					background: 'var(--card)', padding: '0.25rem', boxShadow: '0 10px 30px rgba(0,0,0,0.4)',
				}}>
					<div style={{ padding: '0.375rem 0.5rem', fontSize: '0.8125rem', fontWeight: 600, opacity: 0.6 }}>
						{dropdownLabel}
					</div>
					<div style={{ height: '1px', margin: '0.25rem -0.25rem', background: 'rgba(255,255,255,0.08)' }} />
					{paletteKeys.map((key) => {
						...
						return (
							<button key={key} type="button" onClick={() => selectAccent(key)}
								style={{
									...
									padding: '0.375rem 0.5rem', borderRadius: '0.25rem', border: 'none',
							...
							>
								...
								<span>{label}</span>
								{currentAccent === key && <span style={{ marginLeft: 'auto', fontSize: '0.75rem', opacity: 0.7 }}>&#10003;</span>}
							</button>
						);
					})}
				</div>
